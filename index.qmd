...
Search and filter the XKCD color list, select colors, build a custom palette, and **download** it as CSV or JSON.

```{ojs}
// Search by name
viewof query = Inputs.search({ placeholder: "Search color name..." })

// Dropdown by hue family
families = ["All", ...Array.from(new Set(allColors.map(d => d.family))).sort()]
viewof family = Inputs.select(families, { label: "Hue family", value: "All" })

// Current filtered options
filtered3 = allColors.filter(d => {
  const q = (query ?? "").trim().toLowerCase();
  const name = (d.color ?? "").toLowerCase();
  const nameMatch = q === "" || name.includes(q);
  const famMatch = family === "All" || d.family === family;
  return nameMatch && famMatch;
})

// Multi-select + add button
viewof toAdd = Inputs.select(filtered3, {
  label: "Select colors to add",
  multiple: true,
  format: d => `${d.color} (${d.hex})`
})

viewof add = Inputs.button("Add to My Palette")
...
```{ojs}
// Handle clicks (Inputs.button value is a click count 0, 1, 2, ...)
if (add) {
  const existing = new Set((palette ?? []).map(d => d.hex));
  const merged   = [...(palette ?? [])];

  for (const c of (toAdd ?? [])) {
    if (!existing.has(c.hex)) merged.push(c);
  }
  mutable palette.value = merged;
  viewof toAdd.value = []; // optional reset
}


// Optional: clear button
viewof clear = Inputs.button("Clear Palette")
if (clear) mutable palette.value = []
...
html`<div style="display:flex; gap:10px; flex-wrap:wrap;">
  ${palette.map(c => html`<div style="width:60px;height:60px;border:1px solid #ddd; background:${c.hex};" title="${c.color} ${c.hex}"></div>`) }
</div>`
```
...
```{ojs}
// Table view
viewof paletteTable = Inputs.table(palette, {columns: ["color", "hex", "rank", "family", "h", "s", "l"]})

// CSV & JSON download links
csvText = d3.csvFormat(palette.map(({color, hex, rank, family, h, s, l}) => ({color, hex, rank, family, h, s, l})))
csvUrl = URL.createObjectURL(new Blob([csvText], {type: "text/csv"}))
jsonText = JSON.stringify(palette, null, 2)
jsonUrl = URL.createObjectURL(new Blob([jsonText], {type: "application/json"}))

html`<p>
  <a href="${csvUrl}" download="my_palette.csv">Download CSV</a>
  &nbsp;|&nbsp;
  <a href="${jsonUrl}" download="my_palette.json">Download JSON</a>
</p>`
```

## Optional: Quick Swatch Grid of Filtered Colors

```{ojs}
// Small grid to view currently filtered choices (not clickable)
cols = 12;
grid = filtered3.map((d, i) => ({...d, col: i % cols, row: Math.floor(i / cols)}))
Plot.plot({
  width: 760,
  height: 20 * (Math.ceil(grid.length / cols) + 3),
  marginLeft: 30, marginBottom: 30,
  x: {axis: null}, y: {axis: null},
  marks: [
    Plot.cell(grid, {x: "col", y: "row", fill: d => d.hex, stroke: "white", inset: 0.5, title: d => `${d.color} (${d.hex})`})
  ],
  caption: "Filtered colors (hover for names). Use the selectors above to choose which to add to your palette."
})
```

## Reflection

- How did the combination of **search**, **hue family**, and a **visual preview** help your palette design workflow?
- What other controls (e.g., lightness range, min saturation) would you add next—and why?
